...
def denoising_periodic(args):
    ...
    a = st.text_input('a: ', '0.25')
    a = float(a.strip())
    b = st.text_input("b: ", "30")
    b = float(b.strip())
    ...
    fig_1 = px.imshow(noisy_image, color_continuous_scale='gray')
    fig_1.update_layout(title='Image Noisy')

    
    fig_2 = px.imshow(norm_image, color_continuous_scale='gray')
